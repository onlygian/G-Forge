...
| **v2.5 — the final G-Forge release, minimal freeze** *(current)* | M47 Planning-Pipeline Honesty ✅ + M48 Review-Pipeline Hardening ✅ (both shipped in v2.4.1) → **M52 — v2.5 Minimal Freeze**, four sessions, hand-cut release 2026-08-30 | 2.5 ships only what survives the G-Proof rebuild or is an adopter-facing bug — filter is the rebuild map's verdict column (`audits/2026-07-rebuild-map.md`). The copy follows reality; the approved-announcement commitment is dissolved (ADR-012 amendment 4, 2026-08-28) |
