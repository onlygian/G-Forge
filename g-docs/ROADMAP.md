...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | M48 COMPLETE (a–e) · e=7a6ea08 · 2026-08-23
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **M48 FAMILY COMPLETE** — M48d shipped `1fc8ace` + pass-close `1ec22f4`; **M48e planned, executed (4 waves / 7 tasks), reviewed, shipped `7a6ea08`**: 3 tier-gate cases (test-check-commit 25→28) · heredoc-pathspec fix in _commit_detect_strip_heredocs (todo task 10 closed+archived; operator token + terminator were the leak, not the body) · HEREDOC-k..o regression cases (test-commit-detect 65→70) · ROADMAP pointer sweep · m48d forecast Outcome backfilled · **attested 617/0/22**, HQ re-summed. · M48e review arc: r1 HOLD (0C/3M/6m + DOCS HOLD 3B/6W) → serialized fix round → r2 HOLD incl. a MANUFACTURED PROVENANCE byline on the reconstructed §H pass record (fix agent stamped wave-agent attribution on code-lead's r1 transcripts) → HQ honest-byline rewrite (false attribution kept on the page as disclosed history, next to its correction) → r3 MERGE READY + DOCS READY at the cap. · Retro g-docs/retros/2026-08-23-m48de-close.md; both forecasts reconciled with evidence tags.
Next up:          · **v2.4.1 candidate intermediate release decision — DEVELOPER'S CALL** (ADR-012 amendment 2026-08-22: M47+M48a–e content; developer cuts, never auto; release sequence per G-RULES §D incl. both manifest versions + CHANGELOG heading + tag). · Then **M51 (Release Reliability)** per build order — carry in: lost-write class (5 sightings, feeds items 1/4), provenance-manufacture rule candidate (reconstructed records must name actual source per transcript block), path-scoped sweep exclusions, resumed-round record convention (r2/r3 write into the r1 file). · Verify-and-archive candidate: todo row 5 (audit Wave C riders) — likely satisfied by M48c/d/e, needs evidence check. · Commits since 45e21ad unpushed (push declined once — developer's call). · CARRIED unchanged: AFK permissions-block decision · M43 W4 governance ADR · n8n slugify · inbox trust-boundary ADR · M38 delivery decision · intake rows (a)-(h) · task 6 · task 7 remainder · version-agreement :41 echo · r5 cosmetic minors · todo row 13 (HEREDOC-p case) · commit-detect comment-duplication/function-length.
Active context:   · Tree clean post-7a6ea08 except this pass-close diff; both sentinels consumed by the M48e commit. · Sentinel stamp is ONE line, space-separated (commit_sentinel_ts=<write-tree> commit_sentinel_head=<HEAD> commit_sentinel_worktree=<toplevel>); stage → stamp → commit separate calls; commit messages via Write-tool file, never heredoc — and the underlying heredoc false-MIXED bug is now FIXED (7a6ea08), so a heredoc message misclassifies nothing, though the Write-tool-file rule stays (stall class is separate). · Sweep exclusions are path-scoped (--exclude-dir), never bare grep -v substrings. · Lost-write class: read-back covers content edits but NOT report files an agent never writes — dispatch prompts must require the agent to read back its own report file by path before DONE. · Reconstructed records name their actual source per transcript block; "as relayed" is not a source; annotate corrections in place, never scrub. · Subagent stalls ~5× over the two passes; SendMessage resume worked every time. · Suite runs >10min: nohup+disown + poll log; never trail a pipe. · LOCAL STATE: review-holds 31 · voice gian · telemetry cautious · last-trim 2026-08-18. · Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass, and never repeat this line's own leading label text anywhere else on it (workflow-checkpoint.sh strips with a greedy BRE through the LAST occurrence; pre-compact.sh uses awk block capture — the two disagree when violated).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
**Version:** ships in the candidate intermediate **v2.4.1** at the M48-family close (per ADR-012 amendment 2026-08-22), and cumulatively in the final **v2.5.0** — patch-class process fixes.
...
**Status:** ✅ COMPLETE (M48a ✅ shipped `df2ca1b` 2026-08-20; M48b ✅ shipped `72fcfd3` 2026-08-21; M48c ✅ shipped `55c5bef` 2026-08-21 — 3 review rounds: r3 HOLD 0C/11M, r4 HOLD 0C/3M, r5 MERGE READY; M48d ✅ shipped `1fc8ace` 2026-08-23 — 3 review rounds + 2 fix rounds, converged at the round cap via HQ doc-class closure + pinpoint re-verify; M48e ✅ shipped `7a6ea08` 2026-08-23 — 3 review rounds + 1 fix round, converged at the cap; family-close v2.4.1 candidate decision pending with the developer) — **split into M48a–M48e on 2026-08-19** (developer decision at `/g-plan`'s budget gate: the monolithic plan estimated ~74 exchanges against a ~33-exchange session budget; split along the wave-dependency structure so each sub fits one session with no mid-plan handoff, each closing through its own review + commit. Execution order is fixed a→e — b's lib overrides must exist before c wires them into fixtures. All subs ship in the candidate intermediate v2.4.1 at the M48-family close and cumulatively in the final v2.5.0 (per ADR-012 amendment 2026-08-22); the split is sequencing only, no scope change. Sub-milestone task detail below is the durable record — the per-sub plan files in `g-docs/plans/` are gitignored scratch.)
...
**Version:** ships in the candidate intermediate **v2.4.1** at the M48-family close (per ADR-012 amendment 2026-08-22), and cumulatively in the final **v2.5.0**.
