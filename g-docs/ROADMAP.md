...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.4.1 RELEASED · 2026-08-24
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **v2.4.1 RELEASE CUT EXECUTED** (developer decision 2026-08-24, per ADR-012 amendment 2026-08-22) — carries M47 + M48a–e + all post-2.4.0 fixes on main. Surfaces updated in one commit: both manifests → 2.4.1 · CHANGELOG `## [2.4.1] — 2026-08-24` (Unreleased promoted intact, intro enumerates full contents) · README version strip + §3c shipped-tags (:44, :50 scoped-sentence, :54) · ROADMAP stamps/status/Version Plan (:517/:535/:545/:547/:667) · g-wiki current-state · **new ADR-012 amendment 2026-08-24**: intermediate cuts mark shipped §3c bullets in place, mixed bullets tag only the shipped sentence — standing rule for every family close. · Gates: suite 617/0/22 attested + HQ re-summed · g-dev fixtures 95/0 (canary proven) · doc gate r1 DOCS HOLD (4B/3W) → 5 of 7 closed, 2 carried → r2 DOCS READY (5/5 swept clean, 2 new advisories) · code gate r1 HOLD (0C/1M/2m — M1 was this handoff itself still calling the cut a pending decision) → fixed → r2 MERGE READY (closures sweep-verified). · **SHIPPED**: release commit `321b809` pushed (`45e21ad..321b809`, carries the 6 pre-cut commits), tag `v2.4.1` pushed, GitHub release live — https://github.com/onlygian/G-Forge/releases/tag/v2.4.1 (body = CHANGELOG 2.4.1 section). · Installed `.claude/hooks/lib/commit-detect.sh` drift realigned from repo source post-release. · Developer removed the project deny rule `Bash(git push*)` from `.claude/settings.json` (classifier blocks self-edit of permission rules; `--force` deny retained). · Todo row 14 added (g-patterns `:313` empty-Unreleased anchor gap, doc r1 W3).
Next up:          · **Release sequence COMPLETE** — remaining §D step 7 only: /g-update on downstream projects (developer's other machines), and locally update the plugin cache via /plugins (2.4.0 → 2.4.1) then re-run /g-update here. · resolve pattern report (g-docs/patterns/latest.md, 27 PENDING) — check g-docs/inbox/adversarial/ first · Then **M51 (Release Reliability)** /g-plan per build order — carry in: lost-write class (5 sightings, feeds items 1/4), provenance-manufacture rule candidate (reconstructed records must name actual source per transcript block), path-scoped sweep exclusions, resumed-round record convention (r2/r3 write into the r1 file). · Verify-and-archive candidate: todo row 5 (audit Wave C riders) — likely satisfied by M48c/d/e, needs evidence check. · CARRIED unchanged: AFK permissions-block decision · M43 W4 governance ADR · n8n slugify · inbox trust-boundary ADR · M38 delivery decision · intake rows (a)-(h) · task 6 · task 7 remainder · version-agreement :41 echo · r5 cosmetic minors · todo row 13 (HEREDOC-p case) · commit-detect comment-duplication/function-length.
Active context:   · Tree clean post-`e7f1319` except the pending pass-close diff: retro g-docs/retros/2026-08-24-v241-release-cut.md (doc gate r1 HOLD 2B/2W → fixed → r2 4/4 closed) · /g-patterns MINE outputs (self-heal rename of the fully-resolved 2026-08-14 report → g-docs/patterns/2026-08-27.md; new g-docs/patterns/latest.md — PENDING count carried on the Next up bullet only) · this handoff. Installed-copy drift resolved (commit-detect.sh realigned from repo source). Plugin cache 2.4.0 → /plugins update then /g-update is the local step 7. · Sentinel stamp is ONE line, space-separated (commit_sentinel_ts=<write-tree> commit_sentinel_head=<HEAD> commit_sentinel_worktree=<toplevel>); stage → stamp → commit separate calls; commit messages via Write-tool file, never heredoc (stall class; the heredoc false-MIXED classifier bug itself is fixed since 7a6ea08). · Sweep exclusions are path-scoped (--exclude-dir), never bare grep -v substrings. · Lost-write class: read-back covers content edits but NOT report files an agent never writes — dispatch prompts must require the agent to read back its own report file by path before DONE. · Reconstructed records name their actual source per transcript block; "as relayed" is not a source; annotate corrections in place, never scrub. · Suite runs >10min: nohup+disown + poll log; never trail a pipe. · LOCAL STATE: review-holds 32 · voice gian · telemetry cautious · last-trim 2026-08-18. · Format note: keep this line's leading label intact, use exactly the three section labels, replace this block **wholesale** each pass, and never repeat this line's own leading label text anywhere else on it (workflow-checkpoint.sh strips with a greedy BRE through the LAST occurrence; pre-compact.sh uses awk block capture — the two disagree when violated).
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
