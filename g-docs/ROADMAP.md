...
```
━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: chore/v2.5-minimal-freeze | v2.5.0 RELEASE COMMIT — this commit is the cut · 2026-08-31
━━━━━━━━━━━━━━━━━━━━
Done this pass:   · Task 20 bump: `plugin.json` + `marketplace.json` → 2.5.0 same commit, `README.md:5`, `g-wiki/README.md:5` current-state, CHANGELOG `[2.5.0]` dateline 2026-08-31. · Task 21 sweep + gate-driven dispositions: shipped-early tags dissolved at README:42 and the §3c source copy (ADR-012 2026-08-24 amendment — tags dissolve at this release), Check-24 declared-local carve-out at README:48 and the §3c source copy (developer-approved), README:772 milestone row → ✅ with ROADMAP M52 flipped ✅ in this same commit, g-wiki:5/:7 + project_brief:39 + comms-plan:8 re-tensed to the 2026-08-31 cut, CHANGELOG:47 cite 369→371, comms plan §7 re-verify rows reconciled (two moot, Check-24 resolved), cut date pinned 2026-08-31 by developer call at publish (the work date, not the tag date), `review-holds` corrected to the currently-unresolved semantics (code r2 finding: D1's evidenced closures were never decremented; live value in the checkpoint banner, not here). · Release-body block order decided (developer, 2026-08-31): 3a → 3b (full) → 3c + the `[2.5.0]` CHANGELOG section; comms plan §4 row + §7 updated. · Native-gate lib `commit-detect.sh` realigned from canonical source, cksum-verified equal to source and `.claude` copy (the one drifted file). · Suite attested 770/0 across 24, HQ-summed (`g-forge-dev-2026-08-31-cut-r1.md`); readme-counts + version-agreement re-run green at code r2. Gate arc: r1 doc DOCS READY 0B/3W · code HOLD 0C/2M/2m → fix round → r2 both lanes HOLD (r1 items unclosed at their §3c source twins, plus fix-round-minted claims; every r1 closure claim that r2 confirmed carries recorded sweep evidence) → r3 both lanes HOLD → the current fix round, which reviews the tree containing this handoff — closing verdicts live in the review records and chat, never in this file.
Next up:          · On both gates green: sentinels → release commit → merge to `main` `--no-ff` → push → tag `v2.5.0` → GitHub release (decided block order) → freeze. · M52 close-out after the release commit: `/g-align` + `/g-telemetry` swarm → `/g-wiki` refresh (closes the usage.md count-twin advisory) → `/g-patterns` → pass-close retro + this handoff rewritten with final verdicts + closure-sweep lines.
Active context:   · Code-gate hold counter: decrements on evidenced closure — live value in the checkpoint banner, not here · telemetry cautious · voice gian · last-trim 2026-08-31. Records (gitignored): `agent-output/g-forge-dev-2026-08-31-cut-r1.md`, `agent-output/review/{code-lead,doc-reviewer}-2026-08-31-v250-cut-r*.md` (the full round series). Advisory carries: D1 set unchanged (code r3 minors incl. tests:190 derive-the-list invariant, doc r4 warnings, F3-era carries, `/g-trim` doc-reviewer memory flag) + cut doc-r2 W2 (`g-wiki/usage.md:18` count twin — owned by the close-out `/g-wiki` refresh) + comms-plan:150 (§3b-vs-R0 boundary — a live publish-time check; §3b ships in full in the release body, hedges intact). Observer gap persists: `bash tests/*.sh` runs unjournaled.
━━━━━━━━━━━━━━━━━━━━
```
