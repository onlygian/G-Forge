...
```
━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: chore/v2.5-minimal-freeze | M52 Session B (fix rounds + telemetry) · 2026-08-29
━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **Wave 3b — all six r1 doc-gate blockers fixed, HQ inline** (M41.md archive-moved to `g-docs/archive/milestones/`; ADR-012 retitled + fully reconciled; ROADMAP nine-milestone chain retired; comms plan, README:42, g-wiki, brief, ADR-010 re-stamped; `g-review` solo-review copy fix). · Pre-gate sweep caught `g-proof-roadmap.md` tracked since 2026-08-21 while five surfaces called it gitignored — fixed. · **G-Sharp field report reproduced against source; Task 23 accepted** (ADR-012 am.4 filter): `review-holds` latch dismantled (`/g-review` owns increment+decrement, `/g-telemetry` never writes it), measurement-vacuum floor `computable < 5` agreeing across spec+skill, retro-heading level fixed, agent roster derived. New `tests/test-telemetry-contract.sh` (23), `EXPECTED_SUITE_COUNT` 22→23, **suite 645/645 across 23, hand-summed**. Falsifiability probe found 3/5 guards green-while-broken; fixed, re-probed, all RED. · **Overrun rule: Tasks 15/18 cut**; Task 15's rule-text half **un-cut at the r2 gate** (developer) — `A-session.md` ×2 + `g-execute` now "~25% of the window used". · **Doc gate r2 (5B/13W) and r3 (3B/12W)** — all blockers fixed on disk incl. `code-lead.md` `AXES:` clauses stamped inert, `167%`→**137%** with all three inputs stated, ADR-012 line cites re-anchored to sections. · `.claude/review-holds` 34→0 with evidence. · Backlog: **context-budget gauge defect** (proxy fired at 40% used; model can't read `/context`).
Next up:          · **`/g-doc-review`** — re-run until DOCS READY; the open blocker at dispatch time is whatever the newest "Doc gate rN" paragraph in Active context names (the ordinal is deliberately not repeated here — a round number written into this line goes stale as soon as the next round returns). History: r7 B1 (`todo.md:5` row 6 counts omitted per ADR-013) + r7 W-1/W-2/W-3; earlier: r5 B1 (`CHANGELOG.md:11` + `patterns-deferred.md:4` retargeted from M51 item 4 to `todo.md` row 6) and r5 W-1..W-5 (`ROADMAP:8` next-up · Task 15 §A7 `[Unreleased]` line · CHANGELOG `Consumers:` + capitalisation · §I `field-reports/` attribution + "two exceptions" · `ROADMAP:9` sweep wording). Ship-as-is set unchanged (see Active context). · ~~`/g-review`~~ **MERGE READY at r3** (r1 HOLD 0C/3M/3m → r2 HOLD 0C/2M/3m → r3 clean; a run is re-attested after every fix round — never reused across one); surface `skills/g-review` · `g-telemetry` · `g-execute` · `agents/code-lead.md` · `rules/g-rules/A-session.md` · `tests/test-run-all.sh` · `tests/test-telemetry-contract.sh`; each round pastes its own `Results:` table. · **Commit** — **mixed** changeset, both sentinels; rewrite the stale scratchpad commit draft. · Then Waves 4–5 (Tasks 6/6b/7/8/9/10/10b/11/12/13), Session C Saturday PM, Session D Sunday release.
Active context:   · Tree dirty and uncommitted on the branch — `git status` is the live list (untracked includes the field report, the forecast, both freeze retros and the new suite); nothing committed on the branch; both sentinels absent. · **Code gate MERGE READY at r3 (2026-08-29)**, 8/8 done conditions, `review-holds` 2→0 on Step 4b evidence; doc gate HOLD at r8, r9 pending on the two r8 fixes. Tier 1 attested three times this session, last run after the r2 code fixes: 645/645, 23 suites, hand-summed each time. · `hooks/workflow-checkpoint.sh:271,:279` still read the retired "remaining capacity" direction — Task 14, Session C; the amber banner contradicts the rule it enforces. · `.claude/rules/A-session.md` installed copy still old wording until `/g-update`. · **Session B closed at 40% window used** after the exchange proxy fired at 15 — on Remote Control `/context` must be pasted back; the session-budget counter is NOT the window. · `review-holds` 0 (decremented 2→0 at code-gate r3 MERGE READY) · voice gian · telemetry cautious · last-trim 2026-08-18. · Retro: `retros/2026-08-29-v25-minimal-freeze-session-b.md`. · **Doc gate r4 (2026-08-29, 2B/14W → HOLD)** — B1 (retired `g-proof-roadmap` gitignored/hand-carry fact at `ROADMAP:625`, `brief:40,:67`, `ADR-012:125,:136`) and B2 (handoff dirty-tree count, omitted per ADR-013) fixed HQ inline; closure sweep (r5 re-ran it repo-wide as `local-only|gitignored|hand-carr|by hand` beside `g-proof-roadmap`): 0 live carriers repo-wide (frozen dated record kept: `retros/2026-08-21:7`; ADR-012 `:42/:60/:64` are dated amendment text, kept; the five `milestones/M29/M33/M34/M35/M44:3` stamps also fixed; `alignment/2026-08-17:14` kept as a dated record). W-6 Task 23 `[Unreleased]` entry added; W-9 `field-reports/` row added to §I; W-15 `CHANGELOG:11` M51→M52. **Ship-as-is (developer, 2026-08-29):** W-10 comms §7 `:130,:147,:148` · W-11 `comms:68,:70` / `README:44,:46` present tense (re-blocks at Session D if Tasks 16/17 miss) · W-12 retro-A frozen figures · W-13 forecast `:4,:21` · W-14 `telemetry-metrics.md` no ship vehicle. **Doc gate r5 (1B/11W → HOLD)** — B1: `CHANGELOG:11` + `patterns-deferred:4` retargeted from M51 item 4 to `todo.md` row 6 (M52 narrowed item 4 away from that half); W-1..W-5 fixed HQ inline. **Doc gate r6 (1B/10W → HOLD)** — B1: `todo.md:5` row 6 itself still said "lands as M51 item 4" (the third carrier of the chain CHANGELOG → patterns-deferred → todo; fixed, chain now ends at the row); W-1 CHANGELOG hook sentence made fold-safe · W-2 §I `field-reports/` `[Unreleased]` bullet added · W-3 Consumers clause narrowed. **Doc gate r7 (1B/10W → HOLD)** — B1: `todo.md:5` row 6's "12 agents / 11 remain / Read/Glob/Grep only" counts drifted (doc-reviewer holds Write since v2.4.1) — omitted, derive at execution (ADR-013); W-1 `ROADMAP:8` ordinal · W-2 attested-run wording · W-3 panel-inert `[Unreleased]` entry — fixed. r8 pending. · **Code gate r1 (2026-08-29, HOLD — 0C/3M/3m; `review-holds` 0→1)** — MAJ-1 `g-review:24` claimed the Pre-review-additions column live (no profile-conditional dispatch exists; the only `debugger`/`error-detective` site is the red-test branch) · MAJ-2 `telemetry-metrics.md:100` + profile-table rows asserted the same unwired fan-out · MAJ-3 `test-review-severity.sh:5-8` header stated the AXES contract as live against `code-lead.md`'s INERT stamp — all three re-stamped inert-as-shipped; minors: `review-orchestrator.md:3` stamped, `test-telemetry-contract.sh:29` marker scoped to the five probed guards. Fixtures 95/0 (g-forge-dev). Drift clean. **Code gate r2 (HOLD — 0C/2M/3m; `review-holds` 1→2)** — MAJ-1 re-minted at `g-review:135` ("dispatched by this skill") · MAJ-2 `g-telemetry:129` worked example still printed "one extra reviewer" · minors `g-execute:22`, handoff `review-holds` figure, tree mutated under the gate (doc r6 landed mid-review — fix rounds now wait for every in-flight gate). All fixed; **Code gate r3 (MERGE READY — 0C/0M/3m)** on a frozen tree with doc r8; all r1/r2 findings closed with sweep evidence; `review-holds` 2→0. · **Doc gate r8 (1B/9W → HOLD)** — B1: CHANGELOG `[Unreleased]` panel bullet told adopters `/g-update` resyncs shipped agents (it handles only `*-architect`/`*-implementer`; `/plugins` is the vehicle) — fixed; W-1 `todo.md:5` derivation clause fixed. **Doc gate r9 (1B/9W → HOLD)** — B1: `ROADMAP:8` still named "round 8" after r8 returned (paired-edit miss when the `/g-review` sibling was struck) — Next-up item made round-agnostic. Carried W-2 (in-flight naming + appended paragraphs) closes at the `/g-retro` handoff rewrite. **Doc gate r10 (1B/9W → HOLD)** — B1: the r9 fix's own parenthetical at `:8` minted a round-count adjective the records do not support (flagged at rounds 5, 7, 8, 9); count omitted, no replacement figure. r11 pending.
━━━━━━━━━━━━━━━━━━━━
```
## Milestones
...
**Status:** 🔄 In progress (2026-08-28 — ADR-012 amendment 4)
**Version:** v2.5.0 — the final G-Forge release, hand-cut per G-RULES §D the way v2.4.1 was on 2026-08-24; no `/g-release` machinery.
**Goal:** The README and the release say only what is true, and 2.5 ships by Sunday 2026-08-30. Filter: the rebuild map's verdict column (`audits/2026-07-rebuild-map.md`) — 2.5 does only what SURVIVES the G-Proof rebuild or is an adopter-facing bug; everything built on a DIES/TRANSFORMS component is dropped to `archive/roadmap-dropped-2026-08-28.md` as a G-Proof candidate, decided at R0.
...
**Scope — four sessions, each opened with `/g-resume` and closed with its gate, a commit, and a `/g-retro` handoff** (plan: `plans/v25-minimal-freeze.md` · forecast: `forecasts/v25-minimal-freeze.md`):
