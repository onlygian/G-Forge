...
```
━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: chore/v2.5-minimal-freeze | Session D1 CLOSED + committed (e75fac0) — the v2.5.0 cut next, fresh session · 2026-08-31
━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **Session D1 closed — row 19 shipped + both release preconditions cleared.** Comms plan read whole (publish-time carries noted); `/g-doctor` 16/16 required green, zero drift, versions aligned — its two advisories (Check 16 template-instantiated class; Check 24 bare-prose in this repo's gitignored CLAUDE.md) explicitly ACCEPTED in the todo-done record. Row 19 fix: quote-aware boundary pad in `hooks/lib/commit-detect.sh` — final form a single-pass `LC_ALL=C` awk scanner (interim bash loop was quadratic, replaced at the code gate), NEWLINE-FREE contract + caller-ordering precondition stated, row-16 sentinel comment de-falsified + pinned, PS-QUOTED-PAD fidelity pins for every boundary character + hand-maintained boundary loop + `GF_PAD_TIMING_MS` guard (probe RED 7325ms vs 6000ms). Row 19 → todo-done; row 20 opened (ANSI-C-quote known limit → G-Proof R0). Weekly `/g-trim` run. Gates: code r1–r2 HOLD → r3 MERGE READY 0C/0M/3m advisory; doc r1–r3 HOLD → r4 DOCS READY 0B/2W advisory. Attestation `g-forge-dev-2026-08-31-d-r2.md` — all 24 suites green, HQ-summed 770/0; g-dev fixtures re-run green post-rewrite. **Committed `e75fac0`** (5 files, both sentinels consumed by the gate).
Next up:          · **The v2.5.0 cut — fresh session (§A7 amber reset taken at the e75fac0 boundary).** Task 20 version bump: `plugin.json:4` + `marketplace.json:12` → 2.5.0 same commit, `README.md:5` version line, `g-wiki/README.md:5` current-state line, CHANGELOG `[2.5.0]` dateline dated at the cut · Task 21 release grep sweep (§D step 5 — grep the literals `2.4.1` / `unreleased` / status words across live surfaces, never a site list) · merge to `main` `--no-ff` + push · Task 22 tag `v2.5.0` + GitHub release, CHANGELOG section as body (gh account holds ADMIN on onlygian/G-Forge — verified) · freeze. Publish-time carries (comms plan §7): announcement block-order decision (developer, open); the re-verify items (the "same verdict" row is dead — no such sentence in the rewritten §3c; check the "recalibrated" wording trap, Check 24 DECLARED-LOCAL imprecision, §3b-vs-R0 boundary). M52 close-out at the release commit: `/g-align` + `/g-telemetry` close swarm · `/g-wiki` refresh (closes the g-wiki/usage.md count-twin advisory) · `/g-patterns` after the swarm · milestone → ✅.
Active context:   · Tree clean at `e75fac0` before this pass-close set (retro `retros/2026-08-31-v25-minimal-freeze-session-d1.md` · this handoff · forecast Outcome rows 1/2 D1 evidence). Ahead of `origin/main` by the full unpushed branch history + this pass-close commit; never pushed, no upstream; sentinels consumed; `review-holds` 2 (code r1 + r2 this session; reset is `/g-telemetry`'s call) · telemetry cautious · voice gian · last-trim 2026-08-31. · Records (gitignored): wave `agent-output/wave-d/task-50-quote-aware-pad.md` + `task-50-fix-r1.md`; attestations `g-forge-dev-2026-08-31-d-r{1,2}.md` + `-d-fixtures-r{1,2}.md`; gate records `code-lead-2026-08-31-d-task50-r{1,2,3}.md`, `doc-reviewer-2026-08-31-d-task50-r{1,2,3,4}.md`; frozen diffs `diff-2026-08-31-d-task50*.patch` (r1–r6). · **Process rules carried into the cut session:** write the minimal true statement or none, and re-read the prose above any code you rewrite — the gate arc surfaced five false/unpinned claims: four minted in fix rounds' own prose, one a pre-existing contract sentence falsified by the code change beneath it (forecast scenario 1, 5th consecutive session; deletion-form fixes closed the arc, rewording never did); re-run any attestation whose exercised path was rewritten after it ran (the fixture leg went stale silently mid-arc); budget-one-resume: one resume for the first implementer, one explicit resume plus its own background re-wakes for the second (journal 5 starts / 3 stops), one for the platform-killed code-lead; permission denial = stop at first block (one HQ attempt, correctly abandoned); disposition findings from verbatim text; HQ-sum every delegate table. · Advisory carries: code r3's three minors (tests:190 completeness universal — durable close is a derive-the-list invariant per code r2:447; two pad-frame wording items), doc r4's two warnings, F3-era carries (code r7 "five reviewers" qualifiers, currency r5 warnings), `/g-trim`'s doc-reviewer memory-curation flag. Installed commit-detect copy realigned from source and hash-verified — clean at `e75fac0` per the doc r2 installed-copy check (postdates the last lib edit). Observer gap persists: `bash tests/*.sh` runs unjournaled.
━━━━━━━━━━━━━━━━━━━━
```
## Milestones
...
**Goal:** The README and the release say only what is true, and 2.5 ships when the F3 audit cycle's gate closes (was "by Sunday 2026-08-30" until the 2026-08-30 amendment below inserted F1–F3 ahead of Session D). Filter: the rebuild map's verdict column (`audits/2026-07-rebuild-map.md`) — 2.5 does only what SURVIVES the G-Proof rebuild or is an adopter-facing bug; everything built on a DIES/TRANSFORMS component is dropped to `archive/roadmap-dropped-2026-08-28.md` as a G-Proof candidate, decided at R0.
