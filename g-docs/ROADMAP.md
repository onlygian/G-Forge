...
```
━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: chore/v2.5-minimal-freeze | F3 CLOSED + committed (81b585b) — Session D (release) next · 2026-08-31
━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **F3 audit cycle closed:** 16 SURVIVES skills read whole by HQ (§A1 override) — 11 findings fixed (1 Major: g-skill-design/router description-list, now a pinned third parity surface) + open ledger executed: rows 5/6/9/14/15/16/18 closed to todo-done (row 9 preserved verbatim there; row 19 opened — see Next up). Scoped Write grants on 5 reviewers + 2 diagnostics (write-instruction-without-grant class extinct — code r7's verified claim); review-orchestrator depth-0 claim retracted + inline return; checkpoint false ADR-nudge block-scoped (§22b, probes RED); commit-detect NL-sentinel false-deny fix (row 16, RED-then-green, HEREDOC-p narrowing blessed); resolved-mainline at 7 sites; g-patterns Step 14 ordered branches; README + agents.md currency re-check executed (6 blockers found and fixed). Attestation 752/0 across 24 suites (HQ-summed). Gates: code r1–r6 HOLD → r7 MERGE READY 0C/0M/2m advisory; doc wave r1–r3 HOLD → r4 DOCS READY 0B/3W; currency r1–r4 HOLD → r5 DOCS READY 0B/6W. **Committed `81b585b`** (50 files, both sentinels consumed by the gate).
Next up:          · **Session D — the release** (per the M52 entry, "Then Session D as written"): read `g-docs/communication-plan-2.5.md` · `/g-doctor` release-hygiene precondition — resolve, or explicitly accept in the record, every finding before the bump · **developer decision first: does todo row 19 (quote-aware boundary pad in commit-detect, hooks surface — a disclosed F3 wave-planning miss, fails toward deny) gate the release, or ride to G-Proof R0?** · Task 20 version bump (`plugin.json` + `marketplace.json`, same commit) · Task 21 release grep sweep (§D step 5 — literals, never a site list) · merge to `main` `--no-ff` + push · Task 22 tag `v2.5.0` + GitHub release with the CHANGELOG section as body · freeze. Nudges due: `/g-trim` (last 2026-08-18) · `/g-align` · M52-close `/g-wiki` (also closes the g-wiki/usage.md count-twin advisory).
Active context:   · Tree clean at `81b585b` before this pass-close set (retro `retros/2026-08-31-v25-minimal-freeze-session-f3.md` · this handoff · forecast Outcome rows 1/2 F3 evidence). Ahead of `origin/main` by the full unpushed branch history + this pass-close commit; never pushed, no upstream; sentinels consumed; `review-holds` 0 (1 up on code r1, 1 down on the r7 MERGE READY) · telemetry cautious · voice gian · last-trim 2026-08-18. · Records (gitignored): wave `agent-output/wave-f3/task-{39..49}*.md`; attestation `g-forge-dev-2026-08-31-f3-r1.md`; gate records `code-lead-2026-08-31-f3-wave-r*`, `doc-reviewer-2026-08-31-f3-wave-r*`, `doc-reviewer-2026-08-31-f3-readme-currency-r*`; frozen diffs `diff-2026-08-31-f3-wave{,-fixround}.patch`. · **Process rules carried into Session D:** a resume message never asserts a delegate's per-item progress (T44/H1 — make the delegate verify its own state); the §B no-new-unpinned-counts hard stop applies to HQ's own record and changelog writes at WRITE time (forecast scenario 1, 4th consecutive session — repeated mints this session, each caught one gate round later); never cite line numbers from a concatenated multi-file read; disposition a finding from its verbatim text, never from memory of it (R2-3 cost three gate rounds); size every dispatch under its agent's cap (implementer 30, doc-writer/doc-reviewer 10 ≈ 6 edits); delivery-scope claims derive from /g-update's own text. · Advisory carries: code r7's two "five reviewers" scope qualifiers (extension sentences sit beside both); currency r5's warnings (agents.md:129 inert-chain qualifier, README RESULT families, "staged file set" shorthand, count twins, agents.md:19/:22 prose). Installed copies realigned from source this session (hooks two rounds, rules, architect agent, architecture skill; .git/hooks/lib included). Observer gap: `bash tests/*.sh` runs unjournaled.
━━━━━━━━━━━━━━━━━━━━
```
## Milestones
