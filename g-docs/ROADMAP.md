...
Next up:          · **F3 audit cycle** (derived from plan/milestone; no explicit directive this session): `/g-resume`, then the remaining SURVIVES skills slice (g-roadmap, g-intake, g-align, g-brief, g-kickoff, g-onboard, g-forecast, g-blast-radius, g-patterns, g-identity, g-wiki, g-tier, g-train, g-skill-design, g-skill-validate, g-roundtable) + open ledger todo 5, 6, 9, 14, 15, 16, 18 (row 11 deliberately outside — M51/G-Proof design input, per the M52 entry) — same cycle shape (audit record → amendment-4 filter → one wave → attestation early → gates → commit → retro). · **Decided 2026-08-30 (developer):** both adds approved, folded into the M52 entry — F3's doc gate includes the README + `g-docs/agents.md` currency re-check; Session D gains the `/g-doctor` release-hygiene precondition before the bump. Developer's bar for F3: all shipped skills and agents spotless — zero unfixed filter-passing findings, both gates green (the M52 "two adds approved" amendment states the full condition). · Then Session D (read `communication-plan-2.5.md` · `/g-doctor` hygiene precondition · Task 20 bump · Task 21 sweep · merge `main` `--no-ff` + push · Task 22 tag + release · freeze). Nudges due: `/g-trim` (last 2026-08-18), `/g-align`, M52-close `/g-wiki`.
