...
```
━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: chore/v2.5-minimal-freeze | M52 F1 closed + committed (5ed9839) · F2, F3, then Session D (release) owed · 2026-08-30
━━━━━━━━━━━━━━━━━━━━
Done this pass:   · M52 scope amended at Session D open: Fable audit cycles F1–F3 inserted before the release (§A1 override for the read, amendment-4 filter per finding); C (DIES/TRANSFORMS pass) → G-Proof R0 first act. · **F1 — commit-gate path**, audit record `g-docs/audits/2026-08-30-fable-f1-commit-gate.md` (committed): F1-1 Critical (native `pre-commit` installed without its `lib/` → every consumer commit denied; was todo 17 "drift") → git-hooks-dir `lib/` install from disk in `g-init` 6a / `g-update` 7a + drift set in `g-doctor` Check 16 / `g-review` Step 1 + installed-layout tests; F1-2 Major (`--no-verify`/`-n`/`core.hooksPath` incl. env forms and every `--no-verify` prefix now denied at PreToolUse); F1-3 Major (eight wrapper shapes detected; `xargs`/GNU-`echo` `-n` tokenizer fix; `${var,,}` → `tr` for bash 3.2); `tests.yml` F6; CHANGELOG `[2.5.0]` Fixed ×4; todo 17/18/15-F6 closed. Gates: code r1–r4 HOLD → r5 MERGE READY 0C/0M/1m; doc r1–r5 HOLD → r6 DOCS READY. Suite 720/0 across 24 suites (HQ-summed), fixtures 82/0, nine probes red. `review-holds` 4→0. **Committed `5ed9839`.** Retro: `retros/2026-08-30-v25-minimal-freeze-session-f1.md`.
...
Active context:   · Tree clean at this pass-close commit; ahead of `origin/main` by `59c7c63` · `7beeabf` · `9dd49c4` · `7208967` · `2633981` · `6592bf2` · `a1a1c2b` · `5ed9839` · this commit; never pushed, no upstream; sentinels consumed; `review-holds` 0 · telemetry cautious · voice gian · last-trim 2026-08-18. · Gate records (gitignored): `agent-output/review/code-lead-2026-08-30-v25-f1-commit-gate-r1..r5.md`, `doc-reviewer-…-r1..r6.md`, `g-forge-dev-2026-08-30-f1-r1.md` + `-v25-f1-commit-gate-fixtures-r1.md`, patches `diff-2026-08-30-v25-f1-commit-gate{,-fixround,-fixround2,-fixround3,-fixround4}.patch`; wave records `agent-output/wave-f1/`; plan Session F1 block (`g-docs/plans/` gitignored). · Installed copies (`.claude/hooks/` ×2, `.git/hooks/` ×2) hand-realigned after the commit, drift 0; `/g-update` not run. · Carried: todo 15 gained the F1 r1 Minor (quoted pathspec with `( ) { }` mutated by the raw-string pad, safe direction); code r5 Minor = gitignored `CLAUDE.md:38` prose; known limit stated in the audit: a commit issued from inside a script file is invisible to argv inspection. · **Process rules carried + added:** whole-file read is per-edit, not per-session (code r2–r4 and doc r1–r5 each left a carrier of a moved fact standing); **never re-type a count as a fix — omit or pin** (`684→719→720` in one session); **disclose every hunk in a fix-round dispatch**; **re-prove a probe when the guard's bound changes**; scratch probes run from a script file, never an inline chain (PreToolUse denies the `git commit` literal; the permission layer denied a scratch `rm -rf`+`sed -i`); F-cycle shape = record first, probe every finding, filter, one wave, separate attestation, both gates. Observer gap: HQ suite runs unjournaled.
━━━━━━━━━━━━━━━━━━━━
```
## Milestones
