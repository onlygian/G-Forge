...
```
━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: main | v2.6.2 RELEASED — M53.1 dogfooding defects · 2026-09-05
━━━━━━━━━━━━━━━━━━━━
Done this pass:   **M53.1 shipped as v2.6.2** — four defects found by dogfooding the plugin on its own repo. (1) `agents/references/` registered as 15 dispatchable agents; fixed by declaring the 19 real agents in `.claude-plugin/plugin.json`'s `agents` array, which turns off the directory scan — chosen over moving the directory so no citation repoints and the three-homes reference layer stays true. Verified live by the developer: `/reload-plugins` dropped exactly those 15 types. Pinned by new `tests/test-agents-manifest.sh` (set equality vs `ls agents/*.md`, falsified four ways including a count-preserving swap). (2) `scaffold.sh` minted a duplicate `M1.md` beside a slugged `M1-foundation.md` — literal path replaced by an `M1-*.md` glob, and the doc gate then caught three surviving carriers in `skills/g-init/SKILL.md` (`:295` reported a file never created, `:306` sent the user to edit it, `:314` was a `## Rules` write-directive that would re-mint the duplicate by hand). (3) `detect-stack.sh` counted ROADMAP keyword matches as detections (12 detections, 11 false, 2026-09-02); `roadmap` dropped as a source. (4) `.claude/voice-profile` diagnosed only — no fix. Suite 1379/0 across 39 (baseline `c24d594` was 1370/0/38). Gates: code-lead r1 HOLD → r2 MERGE READY → r6 re-anchor; doc-reviewer r3 HOLD (6 blocking) → r4 HOLD (1) → r5 DOCS READY; release gate code r1 HOLD (1 major) → r3 MERGE READY, doc r2 HOLD (2 blocking) → r4 DOCS READY.
Next up:          · **Consumers:** `/plugins update`, then `/g-update` to resync installed copies. · **M54 (Wiki & README Currency) is on branch `m54-wiki-readme-currency`, not main** — its tip is a `git stash -u` commit, so it carries the README 801→388 rewrite and four rebuilt wiki pages in its tree, but **NOT** `g-wiki/reference.md` (the milestone's main new deliverable) or `g-docs/forecasts/m54-wiki-readme-currency.md`: a `stash -u` keeps untracked files in a third parent, and those two live only in `m54-wiki-readme-currency^3` (`3d6011d`). **Both refs are pushed** — `origin/m54-wiki-readme-currency` (tip: README rewrite + four wiki pages) and `origin/m54-untracked-rescue` (`3d6011d`: those two files) — verified with `git ls-remote --heads origin` and `git ls-tree -r` against each. **Fold them into the tip before doing anything else with this branch** (`git restore --source=m54-wiki-readme-currency^3 -- g-wiki/reference.md g-docs/forecasts/m54-wiki-readme-currency.md`, then commit through the gate) — a plain checkout of the tip silently produces neither file. M54 also has its own 16-blocking DOCS HOLD still open (verdict is **committed** at `g-docs/audits/2026-09-02-m54-doc-review-verdict.md`, promoted 2026-09-05 from the gitignored `g-docs/agent-output/` path that would not have survived this working copy — read it as of its own date, since v2.6.2 has since changed some of the surfaces it discusses, and re-gate rather than working the list blind). Rebase it on this release before resuming; it predates M53.1 and its README edits collide with the 2.6.2 version bump at `README.md:5`. · **Defect 4 needs a developer DoD** before its fix can be specced: the diagnosis is **committed** at `g-docs/audits/2026-09-03-voice-profile-diagnosis.md` (promoted 2026-09-05 from a gitignored path): the general conversational surface has no read site; several skills read the file and some branch their own output on it — re-derive that set from source at fix time rather than trusting any enumeration, including the one in that record, which was refuted repeatedly during the release gate. G-RULES §B still asserts a contract nothing implements. · **`/g-intake` the G-Cosmos/G-Forge separation** raised 2026-09-03 (stale `~/.claude/plugins/cache/g-cosmos/g-forge/0.1.0/` cached under the g-forge name; bare `g-forge:` namespace prefix cannot disambiguate two marketplaces; G-Cosmos is a `directory` source, G-Forge a pinned `github` source). · Backlog carried from v2.6.0: token accounting, generate-on-miss cards, tier-downgrade replays, rules-chain deeper cut, mid-size skill diets, banner-hash reaper.
Active context:   **Two open defects in G-Forge's own gate machinery, both found by running it on itself this pass, neither triaged into a milestone.** (a) `skills/g-review/scripts/build-review-pack.sh` cannot see **untracked** files. Two sites, both tracked-only by construction: `compute_tree()` builds `PACK_TREE` with `git add -u` (`:92`, `:94`), which stages modifications to tracked files and nothing else; the file set comes from `git diff --name-only "$DIFF_BASE"` with `DIFF_BASE=HEAD` (`:167`, `:173-174`). `tests/test-agents-manifest.sh` was absent from the r1 pack until it was staged by hand, so any milestone adding a new file can have it silently skipped by code review. (b) `tests/test-review-pack.sh` is **nondeterministic under `run-all.sh`** — it has been observed failing 2 assertions on an unchanged tree and passing 53/0 in isolation on the same tree. No reliable failure rate was recorded; treat the rate as unknown rather than trusting a figure. An intermittent suite makes every attestation soft. · **Carried, deliberately unfixed in 2.6.2:** `README.md:559` still lists `g-docs/milestones/M1.md` (same staleness class as the `skills/g-init/SKILL.md` fixes, left because README is mid-rewrite on the M54 branch — sweep it there); `profiles/claude-plugin/rules/architecture.md:6` and its installed twin do not yet state that a new agent must be listed in `plugin.json` `agents[]`, non-blocking because `tests/test-agents-manifest.sh` goes red on the omission; no ADR for the manifest-`agents` decision, whose rationale currently lives in that test's header comment. · **Process note worth keeping:** most blocking doc findings this pass were false or unsupported claims in records HQ itself wrote — a release-note roster figure (41) HQ had already recorded as unreconciled before publishing it; two greps reported as proving repo-wide absence when they only pinned the working tree (one run while M54 was stashed); and a branch-state claim written into this very handoff after the commit that would have made it true had been rejected by the commit gate, unnoticed. The gates caught all of it; nothing shipped. The recurring shape is a conclusion stated beyond what its instrument measured — see G-RULES §H's instrument-claim rule, which names exactly this.
━━━━━━━━━━━━━━━━━━━━
```
