...
```
━━━━━━━━━━━━━━━━━━━━
HANDOFF — g-forge | branch: chore/v2.5-minimal-freeze | M52 gates pass 1 closed · 2026-08-29
━━━━━━━━━━━━━━━━━━━━
Done this pass:   · **Session A/B changeset committed — `59c7c63`, 35 files, first commit on the branch** — after doc gate r4–r11 (DOCS READY at r11, 0B/10W) and code gate r1–r3 (MERGE READY at r3, 8/8 done conditions) on one frozen tree; Tier 1 attested three runs, 645/645 across 23 suites hand-summed each time; g-dev fixtures 95/0; drift clean. · Shipped in it: Task 23 (review-holds single-owner, measurement vacuum, derived roster, new suite), Task 15 rule-text half (§A7 "~25% of the window used"), solo-panel disclosure (`g-review` Step 0, `code-lead`, `review-orchestrator`, telemetry profile table, `test-review-severity` header), G-RULES §I `field-reports/` row, four `[Unreleased]` entries. · Record: retired `g-proof-roadmap` gitignored/hand-carry fact swept at ten sites; deferred scoped-Write pattern retargeted along its chain (CHANGELOG → patterns-deferred → `todo.md` row 6, counts omitted); `.claude/review-holds` 0→1→2→0 under the new contract. · Retro: `retros/2026-08-29-v25-minimal-freeze-gates-pass1.md` (uncommitted, with this block).
Next up:          · **Commit this retro + handoff** (doc-only → `/g-doc-review` → docs sentinel → commit), or fold into the next gated commit. · **`/g-update`** — installed `.claude/rules/g-rules-A-session.md` + `g-rules-I-project-tracking.md` lag source (tag precondition, §D step 9). · **Session B Waves 4–5 — `/g-execute 4`**: Tasks 6, 6b, 7, 8, 9, 10, 11, 12, 13 (`plans/v25-minimal-freeze.md`); gate + commit. · **Session C (Sat PM)** — Wave 6: Task 14 (`hooks/workflow-checkpoint.sh:271,:279` + `tests/test-workflow-checkpoint.sh:462` as a *direction* assertion), forecast relabel, M40 REFERENCE classifier + tests + probe; Wave 7: local arch-skill copy; Wave 8: CHANGELOG `[2.5.0]` folding the `[Unreleased]` entries (Task 19 done condition "Tasks 1–23"). · **Session D (Sun 2026-08-30)** — read `communication-plan-2.5.md` first; bump, §D step 5 grep sweep, tag `v2.5.0`, release, freeze. Session D re-block conditions from the doc records: `comms:68,:70` / `README:44,:46` present tense for Tasks 16/17; `CHANGELOG:28` distributive attribution.
Active context:   · Tree clean after `59c7c63` except the retro + this block; branch never pushed, no upstream; both sentinels cleared by the post-commit hook. · `review-holds` 0 · telemetry cautious · voice gian · last-trim 2026-08-18 (`/g-trim` and `/g-align` nudges active). · **Ship-as-is (developer, 2026-08-29):** comms §7 `:130,:147,:148` · comms/README present tense for Tasks 16/17 (re-blocks at Session D if they miss) · retro-A frozen figures · forecast `:4,:21` · `telemetry-metrics.md` no ship vehicle. · Carried doc WARNINGs at r11, all advisory: `CHANGELOG:28` distributive attribution · forecast `:20` stale deferral target · round-ordinal ID drift in two frozen records · retro-B "all five" · retro-B dirty-tree copy · `ADR-012:36` cite · `tests/README.md:1` "Hook tests" · gitignored plan Wave Schedule. · **Process rules that came out of this pass:** fix rounds wait for every in-flight gate (tree frozen while a gate runs); a fix writes zero new facts — read the whole target row/sentence before pointing at it; `perl -i` `$`-anchors no-op on CRLF files, verify every substitution by count; the model cannot read `/context` — on Remote Control paste it back at amber (fired at exchange 15 here; session closed at ~19 to reach the commit boundary). · Observer gap: HQ `bash tests/run-all.sh` runs are not journaled as `test` events. · Gate records for this pass: `agent-output/review/doc-reviewer-2026-08-29-v25-minimal-freeze-r3..r10.md`, `code-lead-2026-08-29-v25-minimal-freeze-r1..r3.md` (gitignored).
━━━━━━━━━━━━━━━━━━━━
```
## Milestones
