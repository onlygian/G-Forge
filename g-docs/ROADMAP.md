...
Next up:          · **Consumers:** `/plugins update`, then `/g-update` to resync installed copies. · **M54 (Wiki & README Currency) is on branch `m54-wiki-readme-currency`, not main** — its tip is a `git stash -u` commit, so it carries the README 801→388 rewrite and four rebuilt wiki pages in its tree, but **NOT** `g-wiki/reference.md` (the milestone's main new deliverable) or `g-docs/forecasts/m54-wiki-readme-currency.md`: a `stash -u` keeps untracked files in a third parent, and those two live only in `m54-wiki-readme-currency^3` (`3d6011d`). **Both refs are pushed** — `origin/m54-wiki-readme-currency` (tip: README rewrite + four wiki pages) and `origin/m54-untracked-rescue` (`3d6011d`: those two files) — verified with `git ls-remote --heads origin` and `git ls-tree -r` against each. **Fold them into the tip before doing anything else with this branch** (`git restore --source=m54-wiki-readme-currency^3 -- g-wiki/reference.md g-docs/forecasts/m54-wiki-readme-currency.md`, then commit through the gate) — a plain checkout of the tip silently produces neither file. M54 also has its own 16-blocking DOCS HOLD still open (verdict was in gitignored `g-docs/agent-output/wave-4/task-6c-doc-review.md` and does **not** survive a fresh clone — re-run `/g-doc-review` rather than hunting for it). Rebase it on this release before resuming; it predates M53.1 and its README edits collide with the 2.6.2 version bump at `README.md:5`. · **Defect 4 needs a developer DoD** before its fix can be specced: the diagnosis stands (the general conversational surface has no read site; several skills read the file and some branch their own output on it — re-derive the set at fix time rather than trusting an enumeration, three of which were wrong this pass), but G-RULES §B still asserts a contract nothing implements. · **`/g-intake` the G-Cosmos/G-Forge separation** raised 2026-09-03 (stale `~/.claude/plugins/cache/g-cosmos/g-forge/0.1.0/` cached under the g-forge name; bare `g-forge:` namespace prefix cannot disambiguate two marketplaces; G-Cosmos is a `directory` source, G-Forge a pinned `github` source). · Backlog carried from v2.6.0: token accounting, generate-on-miss cards, tier-downgrade replays, rules-chain deeper cut, mid-size skill diets, banner-hash reaper.
