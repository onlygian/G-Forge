...
|----------|-----------|---------------------|-------|
| 1 | yes | **happened** — three consecutive rounds (r1 6B, r2 5B, r3 3B); every round's blockers were twins of the prior round's fix class, e.g. README:42 fixed while comms-plan:66 survived, g-execute:156 conditional flipped while "Above the floor, proceed." survived | record: pre-gate sweeps ran each round and caught one unscheduled stale fact per round (g-proof-roadmap tracking, g-telemetry:131 roster count), but the twin-in-same-file class still reached the gate every time. Instrument lesson recorded at plan Task 15: phrase-grep ≠ direction-grep  · gates pass (2026-08-29): doc r4–r11 continued it — every blocker from r5 on was minted by the previous round's fix (retro 2026-08-29-v25-minimal-freeze-gates-pass1) · **F1 (2026-08-30): happened again** — code r2→r3→r4 each closed a finding and minted the next carrier of the same fact (CI trigger; `684→719→720` typed total), doc r1–r5 the same on contract blocks and dates; broken only by omitting the count instead of re-typing it (retro `2026-08-30-v25-minimal-freeze-session-f1.md`) |
| 2 | yes | **did not happen** — session ran zero implementation delegates; HQ applied all fixes inline after Session A's yield rate. Both doc-reviewer dispatches wrote their record on the first attempt | mitigation-held: every reviewer dispatch named an output_file and was told to write it first; HQ verified both paths on disk before reading (record)  · gates pass (2026-08-29): 12/12 dispatches wrote their record first attempt (8 doc-reviewer, 3 code-lead, 1 g-forge-dev; journal start/stop pairs) — mitigation held again |
...
| 4 | yes | **did not happen** — Task 14 ran in Session C (2026-08-30): the hook's amber echo was rewritten to the USED direction and the direction-pinned `check_match` plus two absence guards went green on the first run (88/0), then were probe-proven RED (85/3) in a scratch copy; no string-pin broke (wave record `wave-6/task-14-a7-wording-pinned-pair.md`, journal: no red run before the commit) | mitigation-held: the test matched the hook's own echo text rather than an eyeballed label, and the guard was neutered in scratch to prove it could fail (git + record) |
