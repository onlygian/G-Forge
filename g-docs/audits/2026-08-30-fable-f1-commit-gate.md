...
- **Code r4 — HOLD 0C/1M/1m** (r3's Minor closed; the Major's last carrier was this record's own doc-r3 bullet, typing the round's figure — reworded to describe the superseded resolution without a number). **Code r5 — MERGE READY 0C/0M/1m** (carried: the gitignored local `CLAUDE.md` prose residue). **Doc r4 — HOLD 1B** (`todo-done.md:90`, the CI-trigger co-site. Doc r4 was dispatched before HQ's single fix round that answered code r2 and doc r3 together; that round rewrote the row while r4 was in flight, so r4 read and reported the stale row; doc r5 verified the closure on the rewritten row), **doc r5 — HOLD 2B/1W** ("unambiguous" at two comment sites after the arm widened; the typed total, already omitted), **doc r6 — DOCS READY 0/0**. Committed `5ed9839`.
