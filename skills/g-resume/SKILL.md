...
- **Handoff** — the `## Active Session` block in `g-docs/ROADMAP.md` (the "Next up" and "Active context" lines). Also read `.claude/compact-state.md` if it exists (the PreCompact snapshot) — it is the same block captured mid-session before a compaction, but nothing deletes it (`hooks/pre-compact.sh` writes it, `hooks/session-start.sh` never removes it), so it can be stale or from another branch. Check it before trusting it: parse its `# Compact State — <timestamp>` header and `## Branch` section, and compare against the ROADMAP handoff's date (the trailing date in the `HANDOFF — … | <date>` title line, or failing that the date of the last commit touching `g-docs/ROADMAP.md`) and the current branch (`git branch --show-current`). If the snapshot's timestamp is older than the ROADMAP handoff's date, or its branch differs from the current branch, print `compact-state.md is stale (<ts>, <branch>) — ignored` and use the ROADMAP `## Active Session` block alone. Compare at the finest granularity both sides offer — the last-commit fallback carries a full timestamp — and when the two sources resolve only to the same day, the ROADMAP block wins and the snapshot is ignored the same way: a same-day tie is supersession, never freshness (the common real case is a mid-session compaction followed by a same-day `/g-retro` handoff rewrite).
