...
EXPECTED_SUITE_COUNT=39  # 19 pre-existing suites + M48c: test-version-agreement.sh + test-router-skill-parity.sh; M48d: test-pre-commit.sh; M52: test-telemetry-contract.sh, test-readme-counts.sh; v2.6 token diet (13): test-g-init-scripts.sh, test-next-adr.sh, test-detect-stack.sh, test-derive-owns.sh, test-g-update-scripts.sh, test-resume-sync.sh, test-review-pack.sh, test-roadmap-scripts.sh, test-g-plan-scripts.sh, test-g-execute-scripts.sh, test-doctor-checks.sh, test-g-patterns-scripts.sh, test-g-forecast-scripts.sh (re-derived at v2.6 wave close, all implementers landed); v2.6 model economy: test-dispatch-matrix.sh; M53.1: test-agents-manifest.sh
actual_count=0
