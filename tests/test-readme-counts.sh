...
#
# Pins three README sentences:
#   README.md:150 — agent/skill/profile counts (19 / 38 / 48+7+1)
#   README.md:277 — /g-forge doctor check total + required/advisory split (25 = 16+9)
#   README.md:382 — agent count repeated in the Agents section (19)
#
...
#
# Profile split derivation (48 stack / 7 combo / 1 supplementary, summing to the 56
# total `profiles/*/` dirs):
...
#
# Doctor check split derivation (16 required / 9 advisory, summing to 25 total):
#   skills/g-doctor/SKILL.md numbers each check as a bold heading "**N. name**" (optionally
#   suffixed "(advisory...)" for checks 17-25, confirmed by Step 1's own prose at SKILL.md:10).
#   Total = count of heading lines; advisory = count of heading lines containing "(advisory";
#   required = total - advisory.
...
# ── Test 1: README.md:150 — agent/skill/profile split sentence ─────────────
...
if [ -n "$LINE150" ]; then
    ...
    if [ "$R_AGENTS" = "$AGENT_COUNT" ] && [ "$R_SKILLS" = "$SKILL_COUNT" ] && \
       ...
       [ "$R_SUPP" = "$SUPPLEMENTARY_COUNT" ]; then
        echo "PASS: README.md:150 agent/skill/profile counts agree with disk ($R_AGENTS/$R_SKILLS/$R_STACK/$R_COMBO/$R_SUPP)"
        PASS=$((PASS+1))
    else
        echo "FAIL: README.md:150 counts disagree with disk — README has agents=$R_AGENTS skills=$R_SKILLS stack=$R_STACK combo=$R_COMBO supplementary=$R_SUPP; disk has agents=$AGENT_COUNT skills=$SKILL_COUNT stack=$STACK_COUNT combo=$COMBO_COUNT supplementary=$SUPPLEMENTARY_COUNT"
        FAIL=$((FAIL+1))
    fi
else
    echo "FAIL: README.md:150 sentence not found (moved, reworded, or missing) — expected shape 'All **N** G-Forge agents, **N** skills, N stack profiles, N combo profiles, and N supplementary profile (frontend-data-flow) become available globally across all your projects.'"
    FAIL=$((FAIL+1))
fi

# ── Test 2: README.md:277 — /g-forge doctor check total + split ────────────
# falsifiability: guard neutered in scratch copy, test confirmed red — 2026-08-29
...
if [ -n "$LINE277" ]; then
    ...
    if [ "$R_DTOTAL" = "$DOCTOR_TOTAL" ] && [ "$R_DREQ" = "$DOCTOR_REQUIRED" ] && [ "$R_DADV" = "$DOCTOR_ADVISORY" ]; then
        echo "PASS: README.md:277 doctor check counts agree with disk ($R_DTOTAL checks, $R_DREQ required + $R_DADV advisory)"
        PASS=$((PASS+1))
    else
        echo "FAIL: README.md:277 counts disagree with disk — README has total=$R_DTOTAL required=$R_DREQ advisory=$R_DADV; disk has total=$DOCTOR_TOTAL required=$DOCTOR_REQUIRED advisory=$DOCTOR_ADVISORY"
        FAIL=$((FAIL+1))
    fi
else
    echo "FAIL: README.md:277 sentence not found (moved, reworded, or missing) — expected shape '/g-forge doctor ->   verify hooks, settings, rules block, and drift - N checks (N required + N advisory)'"
    FAIL=$((FAIL+1))
fi

# ── Test 3: README.md:382 — agent count repeated in Agents section ─────────
# falsifiability: guard neutered in scratch copy, test confirmed red — 2026-08-29
...
if [ -n "$LINE382" ]; then
    ...
    if [ "$R_AGENTS382" = "$AGENT_COUNT" ]; then
        echo "PASS: README.md:382 agent count agrees with disk ($R_AGENTS382)"
        PASS=$((PASS+1))
    else
        echo "FAIL: README.md:382 agent count disagrees with disk — README has $R_AGENTS382, disk has $AGENT_COUNT"
        FAIL=$((FAIL+1))
    fi
else
    echo "FAIL: README.md:382 sentence not found (moved, reworded, or missing) — expected shape '**N** agents ship with every install. Full reference: [g-docs/agents.md](g-docs/agents.md)'"
    FAIL=$((FAIL+1))
fi
